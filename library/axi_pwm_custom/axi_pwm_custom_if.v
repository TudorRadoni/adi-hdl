...
module axi_pwm_custom_if ( 
  ...
  reg     [11:0]  curr_data_0, curr_data_1, curr_data_2, curr_data_3;
...
// Create a counter from 0 to PULSE_PERIOD
  ...
  always @(posedge pwm_clk) begin
   ...
   if (end_of_period) begin
    counter <= 0;
    curr_data_0 <= data_channel_0;
    curr_data_1 <= data_channel_1;
    curr_data_2 <= data_channel_2;
    curr_data_3 <= data_channel_3;
   end else begin
    counter <= counter + 1;
   end
...
// control the pwm signal value based on the input signal and counter value
  ...
  always @(posedge pwm_clk) begin
    ...
    if (counter > curr_data_0) begin
      pwm_led_0 <= 0;
    end else begin
      pwm_led_0 <= 1;
    end

    if (counter > curr_data_1) begin
      pwm_led_1 <= 0;
    end else begin
      pwm_led_1 <= 1;
    end

    if (counter > curr_data_2) begin
      pwm_led_2 <= 0;
    end else begin
      pwm_led_2 <= 1;
    end

    if (counter > curr_data_3) begin
      pwm_led_3 <= 0;
    end else begin
      pwm_led_3 <= 1;
    end
